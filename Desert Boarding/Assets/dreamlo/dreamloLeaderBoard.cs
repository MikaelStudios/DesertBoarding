...
public class dreamloLeaderBoard : MonoBehaviour
{
    ...
    public bool AddScore(string playerName, int totalScore)
    {
        var Scores = ToListHighToLow();
        if (Scores == null) return true;
        foreach (var ascore in Scores)
        {
            string s = ascore.playerName.Replace("+", " ");
            if (playerName == s && totalScore == ascore.score)
                return false;
        }
    ...
    public IEnumerator GetScoresDelay()
    {
        highScores = "";
        yield return StartCoroutine(GetRequest(dreamloWebserviceURL + publicCode + "/pipe"));
    }
